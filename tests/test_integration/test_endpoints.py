...
POST_SIMPLE = [
    ...
    (
        "/strains",
        {"name": "strain", "genotype": "", "parent_id": None, "organism_id": 1},
    ),
    (
        "/bioentities",
        {"name": "reaction1", "namespace_id": 1, "reference": "666", "type_id": 1},
    ),
]
...
def test_get_all(client, db, tokens_read, endpoint):
    ...
    for token in tokens_read:
        resp = client.get(
            endpoint, headers={"Authorization": "Bearer {}".format(token["token"])}
        )
        assert resp.status_code == 200
        results = resp.get_json()
        assert set([i["project_id"] for i in results]) <= set(
            token["projects"] + [None]
        )
...
def test_get_one(client, db, tokens_read, endpoint):
    ...
    for token in tokens_read:
        ...
        resp = client.get(
            endpoint + "/{}".format(private_object["id"]), headers=headers
        )
        assert resp.status_code == 200
        result = resp.get_json()
...
def test_crud_public_data(client, db, tokens_write, endpoint):
    ...
    response = client.put(
        f"{endpoint}/{object_['id']}", data=json.dumps(object_), headers=headers
    )
    assert response.status_code == 403
...
def test_post_put_delete(client, db, tokens_admin, pair):
    ...
    admin_tokens = [
        ...
        if all([level in ("admin",) for level in t["claims"].values()])
    ]
    ...
    for project_id in project_ids:
        for token in admin_tokens:
            ...
            if project_id not in token["projects"]:
                ...
                resp = client.delete(
                    endpoint + "/{}".format(result["id"]), headers=headers
                )
                assert resp.status_code == 200
                ...
                resp = client.post(
                    endpoint, data=json.dumps(new_object), headers=headers
                )
...
def test_cross_project_strain(client, db, tokens_write):
    ...
    )  # no access to the project the linked object belongs to
...
def test_medium(client, db, tokens_admin):
    ...
    medium_info = {
        "project_id": tokens_admin[0]["projects"][0],
        "name": "medium1",
        "ph": 3,
    }
    ...
    def get_compounds(ids):
        return [
            {"id": i, "mass_concentration": j} for i, j in zip(ids, [0.1, 0.2, 0.3])
        ]
    ...
    assert (
        resp.status_code == 404
    )  # not all the linked biological entities are compounds
    ...
    resp = client.put(
        "/media/{}".format(medium_id), data=json.dumps(medium_info), headers=headers
    )
    assert resp.status_code == 200
    ...
    for compound_id in compounds_correct:
        assert (
            client.get(
                "bioentities/{}".format(compound_id), headers=headers
            ).status_code
            == 200
        )
...
def test_condition(db, client, tokens_admin):
    ...
    for collection in itertools.product(
        experiment.items(), medium.items(), strain.items()
    ):
        ...
        resp = client.post(
            "/experiments/{}/conditions".format(experiment_id),
            data=json.dumps(condition_info),
            headers=headers,
        )
    ...
    for item in itertools.chain(
        ...
        resp = client.put(
            "/conditions/{}".format(new_condition["id"]),
    ...
    headers.pop("Content-Type")
    resp = client.get(
        "/experiments/{}/conditions".format(new_condition["experiment_id"]),
        headers=headers,
    )
    assert resp.status_code == 200
...
def test_samples(db, client, tokens_admin):
    ...
    for key, value in condition.items():
        resp = client.post(
            "/conditions/{}/samples".format(value),
    ...
    for collection in itertools.product(
        numerator.items(), denominator.items(), condition.items()
    ):
        permissions, (numerator_id, denominator_id, condition_id) = list(
            zip(*collection)
        )
        permissions = list(set(permissions))
        data = {
        ...
        resp = client.put(
            "/samples/{}".format(obj["id"]), data=json.dumps(data), headers=headers
        )
        if len(permissions) == 1 and permissions[0] == "correct":
            assert resp.status_code == 200
        else:
    ...
    headers.pop("Content-Type")
    resp = client.get(
        "/conditions/{}/samples".format(condition["correct"]), headers=headers
    )
    assert len(resp.get_json()) > 0
    assert resp.status_code == 200
